...
class BackBoneLayer(nn.Module):
	...
	def __init__(self,pre,out_feats):
		...
		if pre == 'resnet101':
			...
			temp_resnet = models.resnet152(pretrained=True)
			self.features = nn.Sequential(*[x for x in list(temp_resnet.children())[:-1]])
		elif pre == 'resnet18':
			temp_resnet = models.resnet18(pretrained=True)
			self.features = nn.Sequential(*[x for x in list(temp_resnet.children())[:-1]])
		self.pre = pre
		...
		# def fun(model,input,output): 
		# 	self.layers_outs.append(output.cpu())

		# for name,a in temp_resnet.named_children():
		# 	a.register_forward_hook(fun)
