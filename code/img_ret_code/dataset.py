...
class TemplateDataset(Dataset):
	...
	def __init__(self,index_file_name,aug=True):
		...
		if aug:
			self.aug_list             = [[indx,x[0],int(x[1]),y] for y in self.AUG_TYPES for indx, x in enumerate(self.index_list)]
		else:
			self.aug_list             = [[indx,x[0],int(x[1]),'I'] for indx, x in enumerate(self.index_list)]
	...
	def __getitem__(self, idx):
		...
		indx,img_path,cat,aug_type = self.aug_list[idx]
